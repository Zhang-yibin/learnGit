//
// Created by yuki on 2024/4/7s.
//

//todo add /matplotlib-cpp.git
#include "rune_predictor.h"
#include <fstream>



RunePredictor::RunePredictor()
{
    ...
    params[2] = 0;
    params[3] = 0;
    YAML::Node config = YAML::LoadFile("/home/yukki/Downloads/sr_vision_2023_06/Rune/Filter/doc/filter_param.yaml");
//    YAML::Node config = YAML::LoadFile("../Rune/Filter/doc/filter_param.yaml");
 ...
 * @return 是否预测成功
*/
bool RunePredictor::predict(double speed, double dist, int timestamp, double &result)
{
...
/*            problem.SetParameterLowerBound(params_fitting,0,0.7);
            ...
            if (rmse > max_rmse)
            {
                cout<<"summary.BriefReport"<<summary.BriefReport()<<endl;
                return false;
            }
            else
            {
