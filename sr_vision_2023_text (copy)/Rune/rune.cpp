...
//    resize(src,src,Size(960,540));
    ...
    if (rune_detector.findCenter(src.img)) {
        ...
        rune_armor = rune_detector.final_armor;
    cout<<"detect color"<<detector.detect_color<<endl;
...
/*float angle = radian * 180 / 3.1415926;*//*
        ...
        if(src.aim_mode==SMALL_BUFF){
            ...
            rune_predictor.predict(spin_speed, r2c_distance, time_sum, pre_angle);
            real_angle += pre_angle * 180 / CV_PI;
        }

        if(real_angle >= 360) real_angle -= 360;

        double r_dis = sqrt((rune_armor.center.x - r_center.x) * (rune_armor.center.x - r_center.x)
                            + (rune_armor.center.y - r_center.y) * (rune_armor.center.y - r_center.y));

        pre_center.x = r_center.x + sin(real_angle * CV_PI / 180) * r_dis;
        pre_center.y = r_center.y - cos(real_angle * CV_PI / 180) * r_dis;

        float dx = pre_center.x - 320.0;
        double p_yaw = atan(dx / 800) * 180 / CV_PI;
        float dy = pre_center.y - 240.0;
        double p_pitch = atan(dy / 800) * 180 / CV_PI;

        circle(rune_info, pre_center, 4, Scalar(0, 255, 0), -1);
        ...
        if (timeSum / 40 > ceres_debug.cols) {
            timeSum = 0;
            ceres_debug = Mat::zeros(500, 1500, CV_8UC3);
        }
        circle(ceres_debug, Point2f(timeSum / 40, 250 + rune_predictor.c_function * 24), 1, Scalar(255, 255, 0), 1);
        circle(ceres_debug, Point2f(timeSum / 40, 250 + rune_predictor.t_speed * 24), 1, Scalar(0, 255, 255), 1);
        circle(ceres_debug, Point2f(timeSum / 40, 250 + pre_angle * 24), 1, Scalar(255, 0, 255), 1);
        ...
        send_data.yaw_angle = predictor.plat_state.yaw_angle - p_yaw -0.4;
        ...
        send_data.pitch_angle = predictor.plat_state.pitch_angle + p_pitch + 90;
...
//       Mat fku=Mat::zeros(Size(640,480),CV_8UC1);
//        circle(fku, rune_armor.center, 2, Scalar(0, 255, 255), 2);
//        imshow("fku",fku);
//seems to unuseless

        circle(rune_info, r_center, 6, Scalar(255, 255, 255), -1);
#endif
...
//        aim_mode=3;
        ...
        spin_speed = (radian - last_radian) / (dt / 1000.0);
        cout<<"zjlsb"<<spin_speed<<endl<<radian<<endl<<last_radian<<"sbzjl"<<endl;
        if(rotation_flag == 0)
...
//            cout<<"                         "<<pre_angle<<endl;
        ...
        line(rune_info, r_center, pre_center, Scalar(0, 255, 255), 2);
        pre_center.y = r_center.y - cos(((real_angle+90) * CV_PI / 180)) * r_dis;
//another another another a shit
//出来是反的 大伙有什么头猪吗。。。
        ...
        //so src/2 maybe is the camera center.

        double p_pitch = atan(dy / 1739) * 180 / CV_PI;
        //p_pitch== diff in the ros
...
//        circle(rune_info, pre_center, 4, Scalar(0, 255, 255), 2);
        circle(rune_info, pre_center, 11, Scalar(223, 255, 255), 8);
...
//        p_pitch -= bulletDroppingCompensation(r2c_distance) - bullet_comp+3.5;
        ...
        //why not cauculate the dropping
        //key
