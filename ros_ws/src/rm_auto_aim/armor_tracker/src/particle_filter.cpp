...
bool ParticleFilter::initParam()
{
  ...
  std::vector<float> read_vector = {0.1};
  ...
  std::vector<float> read_vector2 = {0.2};
  initMatrix(observe_noise_cov_tmp, read_vector2);
  ...
  if (is_ready) {
    ...
    for (int i = 0; i < matrix_particle.cols(); i++) {
      ...
      Eigen::MatrixXd weights_dist =
        (matrix_particle.col(i) - mat_measure.col(i)).rowwise().squaredNorm();
      Eigen::MatrixXd tmp =
        ((-(weights_dist / pow(sigma, 2)) / matrix_particle.cols()).array().exp() /
         (sqrt(CV_2PI) * sigma))
          .array();
