...
//#define filename "/home/yukki/0307.mp4";//较为实际/
#define filename "/home/yukki/LIT FULL.mp4";//较为好看
//#define filename "/home/yukki/true2.mp4";


//      FIX ME :扇叶跳变的时候有一点点卡顿。。
//#define DRAW
#define delay 50
...
const static cv::Mat kern11 = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(11, 11));
...
//? 蓝色的参数，建议上网改改
            if(h < 10 || h > 170 || s < 100 || v < 100) {
                hsv.at<cv::Vec3b>(i, j) = cv::Vec3b(0, 0, 0);
            }
#endif
        }
    }
...
//实践得出结论：亮度高时均接近白光，反向通道反而可以滤掉圆心，有一定b用
    if (rune_color == self_BLUE)
        binary_img = channels.at(2)-channels.at(0);
...
//    binary_img=channels.at(0);
    ...
    //为了处理背景光 真得通道分离吧
    //单通道近似灰度图
    imshow("通道二值化old",binary_img);

//    Mat redimage = channels.at(2) - channels.at(0);
//    imshow("通道二值化new",redimage);
    threshold(binary_img, binary_img, rune_param.binary_threshold, 255, cv::THRESH_BINARY);

    morphologyEx(binary_img, binary_img, cv::MORPH_DILATE, kern3);
    //核大丢灯条且失精度
    //但是丢的灯条一般是最底下那个，所以直接统一不算那个就行？
#ifdef show_debug
...
//        imshow("filled_contour_img", filled_contour_img);
...
//话说这个画的是啥，怪，还是很怪
        circle(rune_debug, lights_roi1_pts2[1], 13, Scalar(20, 200, 255));
        circle(rune_debug, lights_roi1_pts2[2], 13, Scalar(20, 200, 255));
        circle(rune_debug, lights_roi1_pts2[0], 13, Scalar(20, 200, 255));
        circle(rune_debug, lights_roi1_pts2[3], 13, Scalar(20, 200, 255));
...
#define text_flowroi
            ...
            for (const auto &contour: flow_roi_contours) {
                ...
                auto rect = cv::minAreaRect(contour);
                circle(rune_debug1, rect.center, 5, cv::Scalar(255, 255, 255), -1);
...
//                    by yuki-mar5，貌似偶尔1near center捏。
                    ...
                    **/
                    ...
                    circle(rune_debug, fana.flow_far_from_center, 7, cv::Scalar(255, 204, 102), 5);
...
//                    cv::Point2f target_center(dst.cols / 2, (start + dst.rows) / 2 - dst.rows / 2);
                    ...
                    circle(debug, roi_target_center, 11, cv::Scalar(11, 123, 0), 2);
...
#define text_moment1
                    ...
                    circle(rune_debug, text_moment, 12, cv::Scalar(255, 204, 102), -1);
#endif
                    //
//                    circle(rune_debug, junction, 8, cv::Scalar(255, 0, 200), 2);//圆心！！！！！！
                    circle(rune_debug, junction, 10, cv::Scalar(255, 255, 255), -1);
                    //这玩意就是最终的击打点了
...
void RuneDetector::find_R(const cv::Mat& frame_src) {
...
//    cv::imshow("blue", frame_blue);
    ...
    cv::Mat kern = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(5, 5));
    cv::morphologyEx(frame_R_binary, frame_R_binary, cv::MORPH_CLOSE, kern);
    cv::imshow("binary_R", frame_R_binary);
//
//前面这一大坨预处理大概都是 shit 不可用。
//好吧其实倒是还行
...
//        // Use area to check if the contour is the R_contour or not
        if (rect.size.area() < max_R_area) {
            ...
            if (min_R_ratio < R_ratio && R_ratio < max_R_ratio && rect.size.area() > min_R_area) {
                ...
                //why we not?cause we are actually full rune
...
//    cv::waitKey(4);
    if (imageProcess(src)) {//一口气全跑完，按理来说只要一个findcenter
        if (fanSizer(fillContour())) {
            ...
            cv::circle(rune_debug, r_coord, 2, cv::Scalar(255), -1);
#ifdef text_moment{
            auto text_moment = final_fan.fan_center - final_fan.towards * final_fan.fan_cols;
                //R歪的话 大概就是towards*col算的不好吧
            ...
            //another key
            //what this fuck 这几把怎么算的 也太sb了。

//why r_coord isn't correct
